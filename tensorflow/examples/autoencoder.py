...
train_y = np.asarray(training_data["BUILDINGID"].map(
    str) + training_data["FLOOR"].map(str))
...
test_y = np.asarray(test_dataset["BUILDINGID"].map(
    str) + test_dataset["FLOOR"].map(str))
...
def train_neural_networks():
    ...
    correct_prediction = tf.equal(
        tf.argmax(predict_output, 1), tf.argmax(Y, 1))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            accuracy_in_train_set = sess.run(
                accuracy, feed_dict={X: train_x, Y: train_y})
            accuracy_in_test_set = sess.run(
                accuracy, feed_dict={X: test_x, Y: test_y})
            print("Epoch: ", epoch, " Loss: ", np.mean(epoch_costs),
                  " Accuracy: ", accuracy_in_train_set, ' ', accuracy_in_test_set)
