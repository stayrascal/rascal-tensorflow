...
class RemoteClient():
    ...
    def decide(self, img_arr, angle, throttle, milliseconds):
        ...
        response = requests.post(self.record_url,
                            files={
                                'img':model.utils.arr_to_binary(img_arr),
                                'json': json.dump(data)
                            })
        data = json.load(response.text)
        angle = int(float(data['angle']))
        throttle = int(float(data['throttle']))
        print('remote client: %s' % response.text)

        return angle, throttle

class RemoteServer():
    '''
    Class used to create server that accepts driving data, records it, runs a predictor and returns the predictions.
    '''
    ...
    def start(self):
        ...
        # laod features
        app = tornado.web.Applicaiton([
            # temporary redirect until vehicles is not a singleton
            (r"/", tornado.web.RedirectHandler, dict(url="/mycar/")),
            ...
            (r"/?(?P<vehicle_id>)[A-Za-z0-9-]+)?/drive/",
                DriveHander,
                dict(pilot=self.port, session=self.session, vehicles = self.vehicles)
            ),
            (r"/static/(.*)", tornado.web.StaticFileHandler, {"path": self.static_file_path}),
        ], debug=True)

        app.listten(self.port)
...
class VehicleHandler(tornado.web.RequestHandler):
    def get(self, vehicle_id):
        ...
        '''
        self.render("template/monitor.html")

class ControllerHander(tornado.web.RequestHandler):

    def initialize(self, vehicles):
        self.vehicles = vehicles

    def post(self, vehcile_id):
        '''
        ...
        # update vehicle angel based on frive mode
...
class DriveHandler(tornado.web.RequestHandler):
    ...
    def post(self, vehicle_id):
        ...
        img_arr = model.utils.img_to_arr(img)
        ...
        # return angle/throttle values to vehicle with json response
...
class CameraMJPEGHandler(tornado.web.RequestHandler):
    ...
    def get(self, vehicle_id, file):
        ...
        self.set_header("Content-type", "multipart/x-mixed-replace;boundary=--boundarydontcross")
        ...
        while True:
            ...
            if self.served_image_timestamp + interval < time.time():
                ...
                img = model.utils.img_to_binary(img)

                self.write(my_boundary)
                self.write("Content-tpe: image/jpeg\r\n")
